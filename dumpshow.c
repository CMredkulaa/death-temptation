...
int main(int argc, char *argv[])
{
    ...
    pcap_t *pcap_handle=pcap_open_live(dev,65535,1,1000,errbuf);
    ...
    if(pcap_compile(pcap_handle,&filter,"dst port 80",1,ipmask)<0){
        printf("error\n");
        return 0;
    }
    ...
    pcap_dumper_t* dumpfp=pcap_dump_open(pcap_handle,"./savetest.pcap");

    while(1)
    {
        if(pcap_dispatch(pcap_handle,20,pcap_callback,(unsigned char *)dumpfp)<0)
        {
            printf("error\n");
            return 0;
        }
