...
#getInfo
cursor = db.cursor()
sql = "select id,srcip,dt,packet_content,body from test where uri='/php/login.php'"
try:
    cursor.execute(sql)
...
for row in result:
    ...
    if(body!="none"):
        username = ''.join(re.findall(r"username=(.+?)&",body))
        password = ''.join(re.findall(r"password=(.+)",body))
